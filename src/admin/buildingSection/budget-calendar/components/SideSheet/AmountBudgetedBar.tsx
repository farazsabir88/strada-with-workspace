...
export default function AmountBudgetedBar(): JSX.Element {
  ...
  useEffect(() => {
    if (singleSideSheetData !== null) {
      if (singleSideSheetData.amount_budget !== null) {
        setAmount(singleSideSheetData.amount_budget);
      } else {
        setAmount(NaN);
      }
    } else {
  ...
  const { mutate: updateAmountBudget, isLoading } = useMutation(async () => axios({
    url: `/api/budget-calendar/event/${singleSideSheetData?.id}/`,
    method: 'PATCH',
    data: {
      // eslint-disable-next-line use-isnan
      amount_budget: Number(amount) === NaN ? 0 : amount,
    },
  }), {
    onSuccess: async (res): Promise<void> => {
      ...
      res.data.amount_budget !== null && enqueueSnackbar('Amount budgeted updated successfully');
    },
  });

  const handleEnterPress = (e: React.KeyboardEvent<HTMLInputElement>): void => {
    if (e.key === 'Enter') {
      if (singleSideSheetData !== null && singleSideSheetData.amount_budget !== amount) {
        if (!(singleSideSheetData.amount_budget === null && Number.isNaN(amount))) {
          updateAmountBudget();
        }
      }
    }
  ...
  return (
    <div className='assignee-sheet-bar'>
      <h6 className='side-sheet-side-label'> Amount Budgeted </h6>
      <div className='assignee-sheet-popover'>
        <div className='popover-btn' aria-hidden='true'>
          {isLoading && amount ? <StradaSpinner open={isLoading} message='Updating' /> : (
