...
interface Ipayload { no_po_approval: number}
...
function POApproval(): JSX.Element {
  ...
  useQuery(
    ...
    }),
    {
      ...
      onSuccess: (approvalData: IDetail) => {
        ...
        setNoApproval(approvalData.no_po_approval);
      },
    },
  );
  ...
  const { mutate: deleteThreshold, isLoading: deleteThresholdLoader } = useMutation(async (payload: number) => axios({
    url: `api/workspace-po-approval/${payload}/`,
    method: 'delete',
  ...
  const { mutate: deleteThresholdApprover, isLoading: deleteThresholdApproverLoader } = useMutation(async (payload: IDeleteThresholdApprover) => axios({
    url: `api/workspace-po-approval/${payload.id}/`,
    method: 'patch',
  ...
  // const { mutate: switchHandler, isLoading: switchHandlerLoader } = useMutation(async (payload: Iswitch) => axios({
  //   url: `api/property-manager-property/${currentBuilding.id}/`,
  //   method: 'patch',
  //   data: payload,
  // }), {
  //   onSuccess: () => {
  //     enqueueSnackbar(' Switched Succsessfully!');
  //   },
  //   onError: () => {
  //     enqueueSnackbar('Failed!', { variant: 'error' });
  //   },
  ...
  // const switchChangeHandler: (e: React.ChangeEvent<HTMLInputElement>) => void = (e) => {
  //   const payload: Iswitch = {
  //     yardi_transactions: e.target.checked,
  //   };
  ...
  return (
    ...
    <div style={{ margin: '1.4rem 1rem 2rem', marginTop: '92px' }}>
      ...
      {/* <StradaLoader open={switchHandlerLoader} /> */}

      <Grid container className='po-container'>
        <Grid item sx={{ width: '33.3rem' }}>
          <Typography className='main-header'>Purchase Order Approval</Typography>
          <Typography className='desc'>This is your approval workflow for purchase order. For changes of this Approval workflow based on the total dollar amount of an PO, setup as many additional workflows you would like.</Typography>
          <Grid container>
            {/* <Grid item md={2}><Switch onChange={switchChangeHandler} /></Grid> */}
            <Grid item md={2}><Switch /></Grid>
            <Grid item md={10}><Typography className='pos-account'>Send Approved POs to Accounting System</Typography></Grid>
          </Grid>
          <Grid container mt={5}>

            <Grid item sm={9}><Typography className='header'> Default Purchase Order Approval Workflow</Typography></Grid>
            <Grid item sm={3}>
          ...
          <Typography className='desc-2'>This is your approval workflow for Purchase Orders.</Typography>
          ...
          <Divider sx={{ marginTop: '.8rem' }} />
          <Typography className='desc-2' mt={2.2} mb={3.5}>For changes to default approval workflow based on the dollar amount of a Purchase Order, setup as many additional workflows you would like.</Typography>
